...
unsigned char crcFail = 0;
unsigned char crcDone = 0;
...
int AiUartRead(ringbuffer_t *handle){
	if(handle->count >= 1){
		...
		if(operationCode == 0x0001){
			...
			controlSingleRelay(pin,state);
			// unsigned char dataSend[8] = {0x55,0xaa,0x06,0x03,0x00,0x04,}
			// respondToAi7688();
			//gpio_write(PORT2,pin,state);
		}

		if(operationCode == 0x0002){
			unsigned char portState = readData[6];
			controlAllRelay(portState);
			//unsigned char dataSend[8] = {0x55,0xaa,}
			//respondToAi7688();
		}

		if(operationCode = 0x0005){
			unsigned char portState = readData[6];
			controlAllRelay(portState);
		}
	}
...
void controlSingleRelay(unsigned char relayID, unsigned char relayState){
	...
	//	relayState&1? ( |= (1<<pin)):(VPORTA.OUT &= ~(1<<pin));
}
