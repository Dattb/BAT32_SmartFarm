...
void IRQ08_Handler(void)
{
		...
		if(INTC_GetPendingIRQ(SR2_IRQn))
		{
			...
			data_tx2 = SCI1->RXD2;
			ring_push_head(&ai7688UartRing,&data_tx2);
			AIReceiveUart = 1;
//			if(Check_Data_setting_485 == 0){
//				Modbus_RTU_Recever(data_tx2);
//			}
//			else{
//				
//			}
    ...
    if(INTC_GetPendingIRQ(SR0_IRQn))
		{
			...
			data = SCI0->RXD0;
			//ring_push_head(&ai7688UartRing,&data);
		}
		...
		if(INTC_GetPendingIRQ(SR1_IRQn))
			{
				...
				data_tx1 = SCI0->RXD1;
				ring_push_head(&modbusUartRing,&data_tx1);
			}
